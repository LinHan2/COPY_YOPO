...
void vins_callback(const nav_msgs::Odometry& msg){

    // 复制时间戳
    curr_pos.header.stamp = msg->header.stamp;
    curr_pos.header.frame_id = msg->header.frame_id; // 通常为 "odom" 或 "map"
    // 复制位姿信息
    curr_pos.pose = odom_msg->pose.pose;
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber local_pos_sub = nh.subscribe<nav_msgs::Odometry>
        ("vin_estimator/odometry",10,vins_callback);
